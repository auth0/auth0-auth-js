...
export class StatelessStateStore extends AbstractSessionStore {
  ...
  async set(
    ...
    chunks.forEach((chunk) => {
      options.response.cookie(chunk.name, chunk.value, cookieOpts);
    });
    ...
    cookieKeysToRemove.forEach((key) => {
      options.response.clearCookie(key);
    });
  }
  ...
  deleteByLogoutToken(): Promise<void> {
    throw new BackchannelLogoutError(
      'Backchannel logout is not available when using Stateless Storage.'
    );
  }
