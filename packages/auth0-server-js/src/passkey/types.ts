 ...
 *
 * Like the MFA and database clients, the passkey client resolves the domain per
 * call so it keeps working in resolver (multi-tenant) mode. It therefore receives
 * the parent client's `resolveDomain` and `getAuthClient` helpers instead of a
