...
/**
 * A state store implementation that stores encrypted state in sessionStorage.
 * Data is cleared when the browser tab/window is closed.
 */
export class SessionStorageStateStore extends AbstractStateStore {
  ...
  async set(identifier: string, state: StateData, removeIfExists?: boolean): Promise<void> {
    ...
    sessionStorage.setItem(identifier, encryptedState);
  }

  async get(identifier: string): Promise<StateData | undefined> {
    const encryptedState = sessionStorage.getItem(identifier);
