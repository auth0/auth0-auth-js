...
import type { FetcherConfig, CustomFetchMinimalOutput } from './types.js';
...
describe('Fetcher', () => {
  ...
  test('should set authorization header correctly', async () => {
    ...
    const setAuthorizationHeader = (fetcher as any).setAuthorizationHeader.bind(
      fetcher
    );
  ...
  test('fetchWithAuth should call the configured fetch function', async () => {
    ...
    expect(mockConfig.tokenProvider).toHaveBeenCalledTimes(1);
  });
  ...
  test('should handle fetch errors gracefully', async () => {
    ...
    await expect(fetcher.fetchWithAuth(request)).rejects.toThrow(
      'Network error'
    );
  });
});
